			...
			//{
				...
				if (goodValues != 0)
				{
					...
					synchronized(obj)
					{
						...
						table.putNumberArray("Azimuths-" + id, i);
					}
...
//			if there is only 1 target, then we have found the target we want
			...
			if(bestResult != null && contours.size() > 1){
				...
				for(int a = 0; a < contours.size(); a++)
				{
					...
					lh = yz - ly;
					Rect rectx = rect;
					if(a < contours.size() - 1)
					{
						rectx = Imgproc.boundingRect(contours.get(a + 1));
					} 
					
					if(Math.abs(rect.x - rectx.x) > lh * 3 || a >= contours.size() - 1)
					{
				...
				Imgproc.rectangle(matOriginal, rec.br(), rec.tl(), new Scalar(256, 256, 0));
				//Imgproc.rectangle(matOriginal, rec.br(), rec.tl(), new Scalar(256, 0, 0));
